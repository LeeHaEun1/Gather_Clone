...
{
    public class LobbyManager_HDRP : MonoBehaviourPunCallbacks
    {
        [Header("Room Name")]
        public GameObject namePanel;
        public GameObject inputRoomNameObj;
        public TMP_InputField inputRoomName;

        [Header("NickName")]
        public GameObject inputNickNameObj;
        ...
        void Start()
        {
            ...
            inputRoomNameObj.SetActive(true);
        ...
        // 닉네임 인풋필드 활성화
        public void EnableNickNameField()
        {
            inputRoomNameObj.SetActive(false);
        ...
        // [2. 닉네임]
        // 닉네임 저장하고, 캐릭터 주위에 해당 닉네임 띠 생성

        // 마이크 & 비디오 설정 활성화
        public void EnableMicVideoSettings()
        {
            inputNickNameObj.SetActive(false);
            inputNickName.enabled = false;
            // 카메라 비디오 설정 on
        ...
        private void Update()
        {
            ...
            {
                if (inputRoomNameObj.activeSelf == true && inputRoomName.text != "")
                {
                    EnableNickNameField();
                }
                else if (inputNickNameObj.activeSelf == true && inputNickName.text != "")
                {
                    EnableMicVideoSettings();
                }
            }
            // Before
            //if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        ...
        {
            // User NickName
            //PhotonNetwork.LocalPlayer.NickName = NickNameInput.text; // Before
            PhotonNetwork.LocalPlayer.NickName = inputNickName.text; // After
